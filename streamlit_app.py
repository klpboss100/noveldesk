...
st.markdown("---")
st.markdown("## STEP 6 — AI 반복 표현 진단")
    ...
    **Claude Pro 구독만 있으면 추가 비용 없이 사용 가능합니다.**
    ...
    if 's6_prompt' in st.session_state:
        ...
        st.download_button("⬇ 프롬프트 TXT 저장", st.session_state.s6_prompt.encode('utf-8'),
                           f"step6_프롬프트_{proj_name}.txt", "text/plain", key="dl6p")
        save_local(report_dir, f"step6_프롬프트_{proj_name}.txt",
        ...
        if s6_paste.strip():
            save_local(report_dir, f"AI진단_{proj_name}.md", s6_paste.encode('utf-8'))
            st.download_button("⬇ 결과 MD 저장", s6_paste.encode('utf-8'),
                               f"AI진단_{proj_name}.md", "text/markdown", key="dl6r")
            if IS_LOCAL and report_dir:
...
with st.expander("**STEP 6+ — AI 진단 자동화** &nbsp;|&nbsp; API 키 보유자용 · 앱 안에서 즉시 처리"):
    st.markdown("""
...
st.markdown("---")
st.markdown("## STEP 7–8 — 고급 분석 (Claude.ai 무료 사용)")
...
with st.expander("**STEP 7 — 복선 탐지 + 회수 확인** &nbsp;|&nbsp; Claude.ai 붙여넣기 (무료 · Pro 구독만 있으면 OK)"):
    st.markdown("""
    **두 단계로 진행합니다.** Claude Pro 구독만 있으면 추가 비용 없이 사용 가능합니다.
    ...
    if 's7_p1' in st.session_state:
                ...
                with st.spinner("프롬프트 생성 중..."):
                    ...
                    if p7b:
                        ...
                        save_local(report_dir,
                                   f"복선탐지_{proj_name}_{r1}-{r2}화_결과.md",
        ...
        if 's7_p2' in st.session_state:
            ...
            fn2 = f"step7_2단계_{proj_name}_{r3}-{r4}화.txt"
            ...
            if s7_paste2.strip():
                fn_r2 = f"복선회수확인_{proj_name}_{r3}-{r4}화_결과.md"
    ...
    **Claude Pro 구독만 있으면 추가 비용 없이 사용 가능합니다.**
    ...
    if 's8_prompt' in st.session_state:
        ...
        st.download_button("⬇ 프롬프트 TXT 저장", p8.encode('utf-8'),
                           f"step8_프롬프트_{proj_name}.txt", "text/plain", key="dl8p")
        save_local(report_dir, f"step8_프롬프트_{proj_name}.txt", p8.encode('utf-8'))
        st.markdown("---")
        ...
        if s8_paste.strip():
            save_local(report_dir, f"출판자료_{proj_name}.md", s8_paste.encode('utf-8'))
            st.download_button("⬇ 출판 자료 MD 저장", s8_paste.encode('utf-8'),
                               f"출판자료_{proj_name}.md", "text/markdown", key="dl8r")
            if IS_LOCAL and report_dir:
